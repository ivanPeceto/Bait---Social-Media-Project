...
export default class Home implements OnInit {
  ...
  deletePostImage(post: Post, media: MultimediaContent): void {
    ...
    this.multimediaService.delete(media.id).subscribe({
      next: () => {
        post.multimedia_contents = (post.multimedia_contents || []).filter(m => m.id !== media.id);
      },
      error: (err) => {
  ...
  saveEditPost(post: Post): void {
    ...
    this.postService.updatePost(post.id, content).subscribe({
      next: (updated: Post) => {
        this.posts = this.posts.map(p => p.id === post.id ? ({ ...p, content_posts: updated.content_posts, updated_at: updated.updated_at }) as any : p);
        this.editingPostId = null;
        this.editContent = '';
      },
      error: (err) => {
          ...
          if (p.user_id === user.id) {
            return {
              ...
              user: {
                ...
                username: user.username ?? p.user?.username,
              }
            } as any;
          }
          return p;
  ...
  /**
   * Gestiona el click en el botón de 'Repost'. Actualiza la UI de forma optimista
   * y luego llama al servicio para crear/eliminar el repost en el backend.
   * @param post El objeto Post que se reposteó/des-reposteó.
   */
  onToggleRepost(post: Post): void {
    const previousRepostCount = post.reposts_count || 0;
    const payload: CreateRepostPayload = {
      post_id: post.id,
    };
