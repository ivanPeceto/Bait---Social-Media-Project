...
{
    // SE ELIMINÓ EL CONSTRUCTOR Y LA PROPIEDAD $guard
     ...
     * )
     */
    public function upload(BannerUploadRequest $request): BannerResource
    {
        $file = $request->file('banner');
        $path = $file->store('banners', 'public'); 

        /** @var User $user */
        $user = $request->user(); // Se usa el método estándar

        $defaultBanner = Banner::where('url_banners', 'banners/default.jpg')->first();

        // Si el usuario tiene un banner Y NO ES el de por defecto, se elimina
        if ($user->banner && (!$defaultBanner || $user->banner->id !== $defaultBanner->id)) {
     ...
     * @OA\JsonContent(
     * @OA\Property(property="message", type="string", example="Banner destroyed. Replaced with default.")
     * )
     * ),
     * @OA\Response(
     * response=401,
    ...
    {
        /** @var User $user */
        $user = auth()->user(); // Se usa el método estándar
